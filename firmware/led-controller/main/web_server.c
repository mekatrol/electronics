...
static const char INDEX_HTML[] =
    ...
    "<h1>LED controller</h1><p>Changes preview immediately on the LEDs. They are written to flash only when you click <b>Save all settings</b>.</p><main id=strings></main><main id=onboard></main>"
    ...
    "<script>let timers={};function colour(x){return '#'+[x.red,x.green,x.blue].map(v=>v.toString(16).padStart(2,'0')).join('')}function card(x,i){return `<section><h2>String ${i+1}</h2><div class=row><label>Physical LED string length<input id=p${i} type=number min=0 max=2048 value=${x.physicalLength}></label><label>LED control length<input id=l${i} type=number min=0 max=2048 value=${x.controlLength}></label><label>Colour<input id=c${i} type=color value=${colour(x)}></label><label>Intensity: <span id=iv${i}>${x.intensity}%</span><input id=i${i} type=range min=0 max=100 value=${x.intensity}></label></div><p class=hint>LEDs from the control length to the physical length are sent black (off).</p></section>`}function onboardCard(x){return `<section><h2>Onboard LED</h2><div class=row><label>Colour<input id=oc type=color value=${colour(x)}></label><label>Intensity: <span id=oiv>${x.intensity}%</span><input id=oi type=range min=0 max=100 value=${x.intensity}></label></div></section>`}"
    "async function refresh(){let s=await(await fetch('/api/state')).json();strings.innerHTML=s.strings.map(card).join('');s.strings.forEach((_,i)=>{for(let id of ['p','l','c','i'])document.getElementById(id+i).addEventListener('input',()=>changed(i))});onboard.innerHTML=onboardCard(s.onboard);for(let id of ['oc','oi'])document.getElementById(id).addEventListener('input',onboardChanged)}"
    ...
    "function onboardChanged(){oiv.textContent=oi.value+'%';clearTimeout(timers.onboard);timers.onboard=setTimeout(onboardPreview,150)}async function onboardPreview(){let v=oc.value,q=new URLSearchParams({red:parseInt(v.slice(1,3),16),green:parseInt(v.slice(3,5),16),blue:parseInt(v.slice(5),16),intensity:oi.value});let r=await fetch('/api/onboard-preview?'+q,{method:'POST'});status.textContent=r.ok?'Preview applied; not saved.':await r.text();return r.ok}"
    "async function save(){Object.values(timers).forEach(clearTimeout);for(let n=0;n<4;n++)if(!await preview(n))return;if(!await onboardPreview())return;let r=await fetch('/api/save',{method:'POST'});status.textContent=r.ok?'All settings saved to flash.':await r.text()}"
...
static esp_err_t preview_onboard_led(httpd_req_t *request)
{
    long red, green, blue, intensity;
    ...
    ESP_RETURN_ON_ERROR(led_controller_preview_onboard(&settings), "web-server", "Could not preview onboard LED");
    return httpd_resp_sendstr(request, "OK");
}
...
{
    led_string_settings_t strings[EXTERNAL_LED_STRING_COUNT];
    led_string_settings_t onboard_settings;
    led_controller_get_settings(strings);
    led_controller_get_onboard_settings(&onboard_settings);
    char response[768];
    size_t used = 0;
